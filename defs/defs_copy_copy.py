...
def getStatisticalCaracteristics(original_df, sensor_column_id, qty_group = 1024, typeof = 'time') :
    ...
    for j in range(int(math.floor(len(timebased_df)/variable_group))) :
        ...
        if (typeof == 'both' or typeof == 'freq') :
            ...
            freq, psdy = scipy.signal.csd(s_new, s_new, fs=1024, window='hann', nperseg=512, noverlap=None, nfft=512, detrend='constant', return_onesided=True, scaling='density', axis=-1, average='mean')
            ...
            freq_features.append(psd_new.std());
